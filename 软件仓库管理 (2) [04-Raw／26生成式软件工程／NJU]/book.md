...
- 听说过 ChatGPT / 一类「和 AI 对话让它写代码」的工具即可
- 知道课程作业可以自己选做、用链接验收（本课约定）
- 本章实验背景零基础可跟
...
作业本身「不限实现方式」：想怎么做就怎么做，最好用 Vibe Coding（用自然语言驱动 AI 写代码的工作方式） 完成。课程会在课上留出时间，现场演示直接让 Agent 做一个，并指出其中的坑。

### 为什么要做这个实验

几乎所有任务都在 AI 的能力范围之内。但如果直接把实验要求丢给 AI，它一开始就会做出许多「看起来合理」的决定，而这些决定会让项目维护越来越困难，最终陷入软件工程里的**焦油坑（项目越改越难维护、越投入越陷进去的泥潭）**。
...
- 知道「项目」不只是代码，还有文档、测试
- 用过或听说过 Git 仓库（会 clone / 看文件即可）
- 若没听过技术债、可追踪性，本章会用例子展开
...
文档、代码、测试之间形成关联，却没有被强行绑定。这正是**技术债 technical debt（现在偷懒，以后要付更高代价去还的不一致）**：随演进，不一致只会越来越多。
...
- 用过 git commit（提交一次改动）
- 知道分支 branch 可以让两个人同时改代码
- 若没用过 rebase / stash，本章按步骤讲，可先当故事听
...
Rebase（把提交搬家、接到新的基线上，常用来整理历史） 试图把一侧的修改「搬到」另一侧之后，得到**线性历史**：
...
graph TD
    ...
    A1 --> R1["rebase（把你的一串提交「搬家」接到别人最新进度后面）: 搬 feature"]
    R1 --> R2["rebase: 搬测试"]
    R2 --> L["线性历史"]
```
...
经典场景：改到一半（编译不过、测试不过），导师突然打电话要修更高优先级 bug。Git 的解法是 `git stash（把改到一半的工作临时藏起来，好先去修别的）`——把工作区压栈。
...
- 读过本册第 3 章，或知道 Git 的 commit 与 rebase 大致在干什么
- 理解「我们真正关心的是修改内容，而不只是快照编号」
- 不必事先会用 jj / Jujutsu
...
- 知道 AI Agent 可以自动改代码、产生很多次修改
- 了解 Git 是「给人用的」协作工具（有 rebase、blame 等）
- 并发 / 锁若未学过，可先看第 6 章再回来
...
即便使用 Git worktree（同一仓库开出多个工作目录，供并行修改） 等 workaround，本质仍是「为人设计的模型」。
...
Git 为人设计的特性还包括 `git blame（查某一行是谁、哪次提交改的——用来追责）`：出问题时回溯是谁的 commit。
...
- 操作系统课：线程、锁是什么（没学过也能按「先上锁再干活」理解）
- 数据库课：事务 transaction、冲突大致是什么（选修）
- 知道 Git 协作里 merge / rebase 可能失败即可
...
- 知道 Vibe Coding ≈ 用自然语言让 AI 写代码
- 了解 Lab1 是「会成长的个人助手」即可
- 本章零基础可跟，重在看「AI 为什么做出你不想要的系统」
...
- **和什么像**：自习座位：乐观=先坐，有人来再让；悲观=先登记锁座。
...
- **常见误解**：MVP 不是偷工减料版终稿，而是**学习与验证的工具**。

### Vibe Coding 与 AI Slop

- **是什么**：Vibe Coding≈自然语言驱动写代码；Slop=AI 量产的空洞/不可靠产出。
...
- **课上为何出现**：解释为何大团队不一定更快，以及为何要拆 team、找能拆任务的人。

### 架构「以不变应万变」

- **是什么**：需求会变时，好架构用尽量少的改动接住变化。
- **课上为何出现**：判断 AI 生成系统好坏，不只看「功能看起来全」。
